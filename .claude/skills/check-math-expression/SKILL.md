...
2. If auto-fixable issues exist (`E005`, `latexmath` → `stem`, `W003` `$$` → `` ```math ``), run `--dry-run` to preview.
...
5. Manually fix anything still reported:
   ...
   - **W001 / W002 / W003** — warnings only; consider removing unnecessary guards, using Unicode, or converting `$$` blocks to `` ```math ``.
...
- **ifdef** (GitHub): Markdown — `$...$`, `` ```math `` (preferred for blocks), `$$...$$` (fallback)
...
### W003 — Prefer ```math over $$
`$$...$$` block math inside `ifdef::env-github[]` is legal but not preferred.
GitHub's Markdown preprocessor can rewrite `_`, `\`, and adjacent CJK before
MathJax sees the formula; fenced `` ```math `` is left untouched. Auto-fix
converts `$$` blocks to `` ```math ``.
...
| `$$` block math (W003) | ✅ | Replaces with `` ```math `` **inside ifdef only** |
